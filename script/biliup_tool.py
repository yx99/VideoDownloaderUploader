...
class BiliBili:
    ...
    def upload_file(self, filepath: str, lines='AUTO', tasks=3):
        ...
        if not self._auto_os:
            ...
            print('[info] '+f"线路选择 => {self._auto_os['os']}: {self._auto_os['query']}. time: {self._auto_os.get('cost')}")
            ...
            if preferred_upos_cdn:
                ...
                if re.match(r'//upos-(sz|cs)-upcdn(bda2|ws|qn)\.bilivideo\.com', original_endpoint):
                    if re.match(r'bda2|qn|ws', preferred_upos_cdn):
                        print('[debug] '+f"Preferred UpOS CDN: {preferred_upos_cdn}")
                        ...
                        print('[debug] '+f"{original_endpoint} => {new_endpoint}")
                        # logger.debug(f"{original_endpoint} => {new_endpoint}")
                        ret['endpoint'] = new_endpoint
                    else:
                        print('[error] '+f"Unrecognized preferred_upos_cdn: {preferred_upos_cdn}")
                        # logger.error(f"Unrecognized preferred_upos_cdn: {preferred_upos_cdn}")
                else:
                    print('[warning] '+f"Assigned UpOS endpoint {original_endpoint} was never seen before, something else might have changed, so will not modify it")
    ...
    async def cos(self, file, total_size, ret, chunk_size=10485760, tasks=3, internal=False):
        ...
        while ii <= 3:
            try:
                ...
                ii += 1
                print('[info] '+f"请求合并分片出现问题，尝试重连，次数：" + str(ii))
        ...
        while ii <= 3:
            try:
                ...
                if res.get('OK') == 1:
                    print('[info] '+f'{filename} uploaded >> {total_size / 1000 / 1000 / cost:.2f}MB/s. {res}')
    ...
    async def kodo(self, file, total_size, ret, chunk_size=4194304, tasks=3):
        ...
        print('[info] '+f'{filename} uploaded >> {total_size / 1000 / 1000 / cost:.2f}MB/s')
    ...
    async def upos(self, file, total_size, ret, tasks=3):
        ...
        while attempt <= 5:  # 一旦放弃就会丢失前面所有的进度，多试几次吧
            try:
                ...
                if r.get('OK') == 1:
                    print('[info] '+f'{filename} uploaded >> {total_size / 1000 / 1000 / cost:.2f}MB/s. {r}')
                ...
                attempt += 1
                print('[info] '+f"请求合并分片时出现问题，尝试重连，次数：" + str(attempt))
            ...
            while True:
                ...
                for i in range(10):
                    try:
                        ...
                        print('[error] '+f"retry chunk{clone['chunk']} >> {i + 1}. {e}")
